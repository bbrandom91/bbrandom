...
date: 2018-08-19
tags: [regression, glm, spline]
excerpt: "Predicting the age of abalone based on their weight"
mathjax: "true"
---
...
Today we will be looking at a dataset from the UCI machine learning repository called the Abalone Data Set. It can be found [here](https://archive.ics.uci.edu/ml/datasets/abalone).

If you take an abalone shell, dry it, and stain it, you will find ring shaped patterns on the inside. Similar to the rings of a tree, the number of rings on the inside of the shell can tell you the age of the abalone (hence, the title of this post). The process of preparing the abalone to actually count the rings is tedious and time consuming, and a more time efficient method is highly desirable. In the dataset we will be looking at, a row represents an abalone, and the columns are the number of rings, some weight measurements, and heighth, length, and width. The goal is to develop a machine learning model that can predict the number of rigns based on these measurements. We will use two approaches. First, we will ignore the discrete nature of ring counts and use a smoothing spline. Second, we will use a technique called Poisson regression, which is a type of generalized linear model (GLM). We could combine these together to get a so-called generalized additive model (GAM), but for now we won't.

A quick note on units: ll lengths in the data set are in millimeters, while all weights are grams. According to the data set description on the website, all continuous values have been scaled down by a factor of 200. 
...
## 2) Clean The Data
I have already added the column names (hence the typo in one of the columns!), and missing values were removed before the dataset was added to the UCI repository.
...
It's just two entries with height 0, so I think it's safe to drop them. While we're here note entry 1174 is a clear outlier. Let's handle that later.
...
All of the features features associated with weight are nearly perfectly correlated with each other, with correlation coefficients > 0.95 for whole weight. This isn't surprising since they're just the weights of different parts of the same abalone. I think we can safely drop the shucked weight, viscera weight, and shell weight and just keep whole weight.
...
Let's try using a GLM for this problem. Rather than treating ring count (or log of ring count) as a continuous variable, let's actually take the fact that they are discrete into account in the model. Define
\begin{equation} 
\lambda = E(Y|X=x)
\end{equation}
for the conditional expectation value of Y, where Y is the ring count. Let's assume that the conditional distribution is Poisson, and we'll use a log link. Then, writing $$ \lambda = e^{f(x)} $$, where f is our model, we have
